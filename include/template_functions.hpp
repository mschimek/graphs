...
inline std::size_t compute_number_edges(const std::string& infile,
  ...
  std::size_t step_size = 2 * num_VId_bytes + num_weight_bytes; // src, dst, weight
...
read_weighted_binaries(const Infiles& infiles, std::size_t num_VId_bytes, std::size_t num_weight_bytes,
                       MPIComm comm = MPIComm{}) {
  ...
  for (const auto& infile : infiles) {
    const std::size_t num_read_edges =
        read_weighted_binary(infile, it, num_VId_bytes, num_weight_bytes, comm);
    it += num_read_edges;
  }
