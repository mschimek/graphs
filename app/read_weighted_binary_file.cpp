...
int main(int argc, char* argv[]) {
  ...
  auto edges = graphs::read_weighted_binaries<graphs::WEdgeList>(infiles, num_vid_bytes, comm);
  graphs::execute_in_order(comm, [&](){ for(const auto& elem : edges) { std::cout << elem << std::endl;}});
  MPI_Finalize();
